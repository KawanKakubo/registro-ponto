        ...
        <div class="mb-4">
            ...
            <select name="establishment_id" required class="w-full border rounded px-3 py-2">
                ...
                @foreach($establishments as $est)
                    <option value="{{ $est->id }}" {{ $department->establishment_id == $est->id ? 'selected' : '' }}>{{ $est->corporate_name }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-4">
            <label class="block font-medium mb-1">Nome *</label>
            <input type="text" name="name" value="{{ $department->name }}" required class="w-full border rounded px-3 py-2">
        </div>
        <div class="mb-4">
            <label class="block font-medium mb-1">Responsável</label>
            <input type="text" name="responsible" value="{{ $department->responsible }}" class="w-full border rounded px-3 py-2">
        </div>
